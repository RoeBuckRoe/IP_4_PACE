/**
 * @file NeoFileReader.cpp
 * @brief Source file for class NeoFileReader
 * @date 11/08/2017
 * @author Andre Neto
 *
 ...
 * the class NeoFileReader (public, protected, and private). Be aware that some 
...
bool NeoFileReader::AllocateMemory() {
    return true;
}

uint32 NeoFileReader::GetNumberOfMemoryBuffers() {
    return 1u;
}

/*lint -e{715}  [MISRA C++ Rule 0-1-11], [MISRA C++ Rule 0-1-12]. Justification: The signalAddress is independent of the bufferIdx.*/
bool NeoFileReader::GetSignalMemoryBuffer(const uint32 signalIdx,
...
const char8* NeoFileReader::GetBrokerName(StructuredDataI &data,
...
bool NeoFileReader::GetInputBrokers(ReferenceContainer &inputBrokers,
...
bool NeoFileReader::GetOutputBrokers(ReferenceContainer &outputBrokers,
                                  const char8 *const functionName,
                                  void *const gamMemPtr) {
    return false;
}
...
void NeoFileReader::ConvertXAxisSignal() {
...
bool NeoFileReader::Synchronise() {
    // Reset fatalFileError to false if returning to reader state after an error
    if (!firsttime){
        fatalFileError = firsttime;
    }
...
bool NeoFileReader::PrepareNextState(const char8 *const currentStateName,
                                  const char8 *const nextStateName) {
    return true;
}

bool NeoFileReader::Initialise(StructuredDataI &data) {
        ...
        if (interpolate) {
            if (!data.Read("XAxisSignal", xAxisSignalName)) {
                REPORT_ERROR(
                        ...
                        "Interpolate=yes and the XAxisSignal was not specified. This will fail if none of the signals interacting with this NeoFileReader has Frequency > 0");
            }
            if (!data.Read("InterpolationPeriod", interpolationPeriod)) {
                REPORT_ERROR(
                        ErrorManagement::Warning,
                        "Interpolate=yes and the InterpolationPeriod was not specified. This will fail if none of the signals interacting with this NeoFileReader has Frequency > 0");
            }
        }
    }
    if (ok) {
...
bool NeoFileReader::SetConfiguredDatabase(StructuredDataI &data) {
...
ErrorManagement::ErrorType NeoFileReader::OpenFile(StructuredDataI &cdb) {
...
bool NeoFileReader::ReadLineCSVFormat() {
    StreamString token;
    char8 saveTerminator;
...
ErrorManagement::ErrorType NeoFileReader::CloseFile() {
...
const StreamString& NeoFileReader::GetFilename() const {
    return filename;
}

const StreamString& NeoFileReader::GetFileFormat() const {
    return fileFormatStr;
}

const StreamString& NeoFileReader::GetCSVSeparator() const {
    return csvSeparator;
}

bool NeoFileReader::IsInterpolate() const {
    return interpolate;
}

const StreamString& NeoFileReader::GetXAxisSignal() const {
    return xAxisSignalName;
}

uint64 NeoFileReader::GetInterpolationPeriod() const {
    return interpolationPeriod;
}

CLASS_REGISTER(NeoFileReader, "1.0")
CLASS_METHOD_REGISTER(NeoFileReader, CloseFile)
